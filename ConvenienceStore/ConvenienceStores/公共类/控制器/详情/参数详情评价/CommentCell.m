...
- (void)initView{
    ...
    _timeLb = [[UILabel alloc]initWithFrame:CGRectMake(Screen_width-75, 23, 65, 15)];
...
-(void)updateFrame{
    ...
    _descLb.frame = CGRectMake(10, CGRectYH(_headIV)+10, Screen_width-20, [self heightWithLabel:_descLb]);
    _infoLb.frame = CGRectMake(10, CGRectYH(_descLb)+5, Screen_width-20, 20);
...
-(CGFloat)heightWithLabel:(UILabel *)sender{
    ...
    CGFloat height = 0.0 ;
    if (str.length*14>Screen_width-20) {
        for (int i=1; i<10; i++) {
            if (str.length*14>(Screen_width-20)*i&&str.length*14<(Screen_width-20)*(i+1)) {
                height =(i+1)*16.0;
            }
        }
    }else{
